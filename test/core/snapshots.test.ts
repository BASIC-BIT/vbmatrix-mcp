...
describe('Matrix snapshots', () => {
  ...
  test('diffs slot metadata and point state', () => {
    ...
    expect(summarizeSnapshotDiff(diff)).toMatchObject({
      slotChanges: { added: 0, removed: 0, changed: 1 },
      pointChanges: { added: 0, removed: 0, changed: 1 },
      totalChanges: 2,
    });
  });

  test('caps diff details for inline responses', () => {
    const before = createMatrixSnapshot({
      capturedAt: '2026-05-24T00:00:00.000Z',
      slots: [{ suid: 'VASIO8', info: 'old', online: '1', runningStatus: '1', master: '0', device: 'A' }],
      points: [
        { target: targetA, state: { dBGain: '-6', mute: '0', phase: '0' } },
        { target: targetB, state: { dBGain: '-12', mute: '0', phase: '0' } },
      ],
    });
    const after = createMatrixSnapshot({
      capturedAt: '2026-05-24T00:00:01.000Z',
      slots: [{ suid: 'VASIO8', info: 'new', online: '1', runningStatus: '1', master: '0', device: 'A' }],
      points: [
        { target: targetA, state: { dBGain: '-3', mute: '0', phase: '0' } },
        { target: targetB, state: { dBGain: '-9', mute: '0', phase: '0' } },
      ],
    });

    const capped = capSnapshotDiff(diffMatrixSnapshots(before, after), 1);

    expect(capped.diff.slots.changed).toHaveLength(1);
    expect(capped.diff.points.changed).toHaveLength(0);
    expect(capped.omittedEntries).toBe(2);
  });
  ...
  test('fails closed when selected restore points are absent from the desired snapshot', () => {
    const desired = createMatrixSnapshot({
      slots: [],
      points: [{ target: targetA, state: { dBGain: '-6', mute: '0', phase: '1' } }],
    });
    const current = createMatrixSnapshot({
      slots: [],
      points: [{ target: targetA, state: { dBGain: '0', mute: '0', phase: '0' } }],
    });

    expect(() => planSnapshotRestore({ desired, current, selectedPoints: [targetC] })).toThrow(
      /not present in the desired snapshot/
    );
  });

  test('reports missing selected restore points when explicitly allowed', () => {
    const desired = createMatrixSnapshot({
      slots: [],
      points: [{ target: targetA, state: { dBGain: '-6', mute: '0', phase: '1' } }],
    });
    ...
    const plan = planSnapshotRestore({ desired, current, selectedPoints: [targetA, targetC], allowMissingSelectedPoints: true });

    expect(plan.summary.pointCount).toBe(1);
    expect(plan.summary.missingSelectedPoints).toEqual([targetC]);
  });
});
