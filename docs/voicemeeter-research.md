...
- Current Matrix query replies observed by this repo are VBAN-TEXT protocol `0x40` on the configured stream or VBAN service protocol `0x60` on stream `Request Reply`; Matrix live behavior uses SERVICE `0x60` on `Request Reply`.
...
- Feed provider-registry requirements into [#16](https://github.com/BASIC-BIT/vbmatrix-mcp/issues/16): product-specific adapters, stable Matrix naming, explicit capability reporting, edition-aware validators, and helper-process failure reporting.
...
- Should a future Windows helper ship inside this package or in a sibling `voicemeeter-mcp` package?
