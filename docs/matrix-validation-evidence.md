...
| Point ranges and zones                                                                          | Partial live-tested + typed implementation | `src/tools/points.ts`, `src/tools/zones.ts`, `docs/matrix-command-coverage.md`                                            | Live MCP stdio execution covered a 1x1 point-range gain write and restore. Zone execution remains dry-run/docs-only; aggregate range/zone state queries remain unverified and should not become implicit high-volume scans. |
| Input/output labels and channel route reset                                                     | Partial live-tested + typed implementation | `src/tools/channels.ts`, `docs/matrix-command-coverage.md`, `docs/research.md`                                            | Live MCP stdio execution set and removed one input channel label. Range label removal and destructive channel route reset remain typed but not live-tested in this pass. Non-empty range assignment is intentionally deferred. |
| Preset patch status and scene operations                                                        | Live-tested read/dry-run + typed implementation | `src/tools/presetPatches.ts`, `docs/matrix-command-coverage.md`, `docs/research.md`                                       | Live MCP stdio read and apply dry-run passed for preset patch 1. File-affecting load/save/save-as remains deferred pending path safety design.                                                   |
...
| Safe route workflow                                                                             | Live-tested + mocked-tested            | `src/core/safeRouting.ts`, `src/tools/safeRouting.ts`, `test/tools/safeRouting.test.ts`                                   | Live MCP stdio execution of `auditionRoute` changed an explicit point and snapshot restore returned it to pre-state.                                                                             |
