...
| Preset patch status and scene operations                                                        | Live-tested + typed implementation     | `src/tools/presetPatches.ts`, `docs/matrix-command-coverage.md`, `docs/research.md`                                       | Live MCP stdio read, dry-run, and live execution covered preset patch 1 name, comment, gain, mute, phase, update, resetZone, apply, recall, copy, paste, and delete. File-affecting load/save-as is covered separately by `vbmatrix_preset_patch_file`; `PresetPatch[n].Save` remains deferred. |
| Matrix file state and preset patch XML load/save-as                                             | Live-tested + mocked-tested            | `src/tools/matrixFiles.ts`, `src/core/matrixFilePolicy.ts`, `test/tools/matrixFiles.test.ts`, `docs/matrix-file-state-safety.md` | Manual evidence records `PresetPatch[i].Load`, `PresetPatch[i].SaveAs`, `Command.Load`, and `Command.LoadGrid`. Live Matrix 1.0.2.6 returned current `Command.Load`/`Command.LoadGrid` state and successfully wrote/loaded an absolute-path `AgentProof_PresetPatch_01.xml` with `PresetPatch[1].SaveAs`/`Load`. Project/grid execution and `PresetPatch[n].Save` remain deferred. |
