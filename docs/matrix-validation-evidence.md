...
| Snapshot capture/diff/restore                                                                   | Live-tested dry-run + mocked-tested    | `src/core/snapshots.ts`, `src/tools/snapshots.ts`, `test/core/snapshots.test.ts`, `test/tools/snapshots.test.ts`          | Live read-only snapshot capture, same-snapshot diff, and restore dry-run passed against Matrix 1.0.2.6. Restore execution remains operator-approved only.                                       |
| Safe route workflow                                                                             | Live-tested dry-run + mocked-tested    | `src/core/safeRouting.ts`, `src/tools/safeRouting.ts`, `test/tools/safeRouting.test.ts`                                   | Live dry-run through MCP stdio returned pre-state and rollback plan for an explicit point. Execution remains operator-approved only.                                                            |
