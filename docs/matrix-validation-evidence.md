...
| -------------------------------------------------------------------------------------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Command.Version=?;` ping and diagnostics                                                    | Live-tested + mocked-tested      | `README.md`, `scripts/vban-diagnostics-scenarios.ts`, `test/core/vbanText.test.ts`, `test/core/vbanTextTransport.test.ts` | Fixed read-only diagnostic command. Live scenario coverage includes configured stream plus intentionally wrong stream and wrong UDP port.                                                 |
| VBAN SERVICE `Request Reply` packet acceptance                                               | Mocked-tested                    | `src/core/vbanText.ts`, `test/core/vbanText.test.ts`, `test/core/vbanTextTransport.test.ts`                               | Accepted as Matrix query replies even though commands are sent on the configured TEXT command stream. Needs continued live capture evidence when operators collect diagnostics artifacts. |
...
| Single `Point(...)` gain/mute/phase query and write                                          | Live-tested + mocked-tested      | `docs/research.md`, `docs/live-audio-verification.md`, `scripts/live-audio-verify.ts`, `test/tools/points.test.ts`        | Live audio harness is opt-in and dry-runs by default. `-inf` maps to `Remove` based on live-observed behavior.                                                                            |
...
| Full-matrix scans, meter streaming, VBAN service/config writes, broad system actions         | Deferred                         | `docs/matrix-command-coverage.md`                                                                                         | Requires bounded scope, safety design, and source-linked command behavior before implementation.                                                                                          |
