...
The MCP has no broad whole-system route scanner, undo stack, or fuzzy device-selection tool. `vbmatrix_inspect_slots` inspects only explicit candidate slots and selected channels, and `vbmatrix_inspect_routes` inspects only explicit selected points. Both are capped. Snapshot tools only capture explicit selected slots and points. Preset patch and zone tools dry-run by default. Voicemeeter tools use numeric strip/bus indexes and explicit parameter names; they do not infer roles such as mic, stream, cue, booth, or main. `vbmatrix_safe_route_workflow` groups a few explicit point operations with snapshot/rollback guardrails; it does not infer DJ intent, output groups, speakers, cue buses, or show-critical routes. Recipes that would benefit from broader tools are marked as future-tool placeholders.
...
## Voicemeeter Read-Only Triage

Use this when the operator asks whether Voicemeeter is available or what a strip/bus is doing.

1. Start with provider status and capabilities.
...
2. Query explicit targets only.

```text
voicemeeter_get_strip({ "index": 0 })
voicemeeter_get_bus({ "index": 0 })
voicemeeter_get_levels({ "levelType": "inputPostMute", "channels": [0, 1] })
```

3. Report unavailable states literally.

- `helper-not-configured`, `missing-install`, and `not-running` are setup states, not mixer states.
- Do not claim a label, role, or physical device is the intended target unless the operator supplied that mapping.
- Use `voicemeeter_get_devices` when the problem is device enumeration or driver visibility.

## Voicemeeter Parameter Write

Use this only when the operator gives an exact strip/bus index, property, and value.
...
2. Apply one explicit parameter.
...
3. Read the response before making claims.

- Treat Remote API acceptance as one fact.
- Treat post-state match or mismatch as a separate fact.
- If helper status changes or post-state is unavailable, say verification failed instead of implying the mixer changed.

## Voicemeeter Device And MacroButton Verification

Use this for #55-style live verification only with an operator-approved disposable fixture.

1. Confirm the target is safe.

- Device assignment/removal can interrupt live audio.
- MacroButtons can run user-configured actions such as scripts, key presses, programs, or routing changes.
- Use an inert/disposable MacroButton for `stateOnly`; do not trigger production buttons.
...
voicemeeter_get_strip({ "index": 0 })
voicemeeter_get_bus({ "index": 0 })
voicemeeter_get_macro_button({ "index": 0 })
```
...
voicemeeter_set_device({
  ...
  "index": 0,
  "driver": "WDM",
  "deviceName": "Operator Approved Test Device",
  "confirm": true
})
```
...
4. Interpret confirmation conservatively.

- `confirmation.writeAccepted` means the Remote API accepted the call.
- Claim durable device or MacroButton state changed only when observed post-state confirms it.
- MacroButton trigger mode can pulse and then read back as `0`; do not treat that as a failed trigger by itself.
- Restore or manually verify the original state after the smoke.

## Voicemeeter VBAN-TEXT Research Probe

Use this only for exact commands while #22 remains open.

1. Confirm setup before probing.

- Voicemeeter is running.
- Voicemeeter VBAN service is enabled.
- The incoming TEXT stream and `VOICEMEETER_VBAN_PORT` are configured separately from Matrix.
- The operator approved the exact command.

2. Send a read-only fixed query first.
...
3. Record packet evidence.

- Sent host, port, and stream.
- `timedOut`, `response`, and `receivedPackets`.
- Ignored packet reasons and timeout classification.
- Whether replies arrive as TEXT `0x40`, SERVICE `0x60` on `Request Reply`, another stream/protocol, or no packets.

Do not build typed Voicemeeter VBAN tools from this path until query/reply behavior is live-verified.
