...
- `vbmatrix_apply_point_range` applies gain, mute, phase, or remove to an explicit point range; it dry-runs by default and requires confirmation to execute.
...
Voicemeeter VBAN-TEXT access is limited to a fixed diagnostic probe and a separate advanced escape hatch named `voicemeeter_raw_vban_text`. Both use `VOICEMEETER_VBAN_HOST`, `VOICEMEETER_VBAN_PORT`, `VOICEMEETER_VBAN_STREAM`, and `VOICEMEETER_VBAN_TIMEOUT_MS`, default to the local separate-port convention `127.0.0.1:6982`, and can be disabled with `VOICEMEETER_MCP_DISABLE_RAW_VBAN_TEXT=true`. Live evidence did not observe a direct Voicemeeter VBAN-TEXT query reply, so typed tools must not depend on VBAN-TEXT replies. Use `voicemeeter_vban_diagnostics` for the fixed read-only query probe before using the raw escape hatch for broader evidence collection.
