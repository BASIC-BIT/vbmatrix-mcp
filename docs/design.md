...
Raw Voicemeeter VBAN-TEXT access is a separate advanced escape hatch named `voicemeeter_raw_vban_text`. It uses `VOICEMEETER_VBAN_HOST`, `VOICEMEETER_VBAN_PORT`, `VOICEMEETER_VBAN_STREAM`, and `VOICEMEETER_VBAN_TIMEOUT_MS`, defaults to the local separate-port convention `127.0.0.1:6982`, and can be disabled with `VOICEMEETER_MCP_DISABLE_RAW_VBAN_TEXT=true`. Query/reply behavior is still treated as unverified for Voicemeeter, so typed tools must not depend on VBAN-TEXT replies until live evidence lands.
