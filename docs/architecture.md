...
- `src/providers/windowsAudio*.ts` defines a third, read-only Windows Core Audio provider. Native COM work stays in the versioned JSON-stdio helper at `helpers/windows-audio-helper.ps1`.
...
- Windows Audio and Voicemeeter use separate helper processes and protocols; neither provider loads native audio APIs into the MCP process.
