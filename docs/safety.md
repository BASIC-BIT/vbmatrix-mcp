...
Windows Audio is currently a read-only provider, so it has no write gate and no raw escape hatch. `windows_audio_get_endpoints` and `windows_audio_get_defaults` run fixed helper operations only. The helper contract binds every response to a generated request ID, limits request/output size and execution time, and terminates the isolated process on timeout. Endpoint IDs, interface paths, policy GUIDs, and device names are local identifiers that should be redacted from shared logs or support reports.
