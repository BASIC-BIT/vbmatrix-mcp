...
- `VBMATRIX_MCP_ALLOW_WRITES=false` disables point and slot writes.
...
- `VBMATRIX_MCP_ALLOW_DESTRUCTIVE=false` disables `vbmatrix_restart_engine`, slot reset, slot device assignment/removal, and any future destructive/system tools.
...
Point and slot write tools should query the affected target before and after sending a command. If a query times out, the write should still report that fact explicitly rather than hiding it.
