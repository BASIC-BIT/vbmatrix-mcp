...
function resolveRoot(root: string): string {
  if (!path.isAbsolute(root)) throw new Error(`Matrix file roots must be absolute paths: ${root}`);
  return path.resolve(root);
}

function assertAbsoluteCandidate(filePath: string): void {
  if (!path.isAbsolute(filePath)) throw new Error('Matrix file paths must be absolute');
}
...
export function validateMatrixFilePath(
  ...
  if (policy.allowedRoots.length === 0) throw new Error(`No allowed Matrix file roots configured for ${request.kind}`);
