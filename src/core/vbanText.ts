...
export class VbanTextTimeoutError extends Error {
  ...
  constructor(command: string, diagnostics: VbanTextExchangeDiagnostics) {
    super(
      `Timed out waiting for VBAN-TEXT response to ${command}. Check VBAN service, incoming TEXT stream '${diagnostics.connection.streamName}', UDP ${diagnostics.connection.host}:${diagnostics.connection.port}, and Matrix '${VBAN_REQUEST_REPLY_STREAM}' replies.`
    );
...
function setupStageHints(diagnostics: VbanTextExchangeDiagnostics): string[] {
  ...
  if (mismatch?.packet.streamName) {
    hints.push(
      `Observed VBAN stream '${mismatch.packet.streamName}' instead of expected command stream '${diagnostics.connection.streamName}' or Matrix reply stream '${VBAN_REQUEST_REPLY_STREAM}'.`
    );
  }
  if (diagnostics.ignoredPackets.some((packet) => packet.reason === 'text_non_utf8')) {
    hints.push('Observed VBAN-TEXT packet with a non-UTF-8 format byte; Matrix command replies should be UTF-8 text.');
  }
...
export function classifyVbanTextPacket(message: Buffer, streamName: string): VbanPacketClassification {
  ...
  if (protocol === VBAN_TEXT_PROTOCOL) {
    ...
    return { accepted: true, reason: 'accepted_text', packet, payload: message.subarray(VBAN_HEADER_SIZE).toString('utf8') };
  }
